...
const get_specific_application = async (res, queryResult, user_id) => {
  try {
    ...
    const statusValue = STATUS.find((status_id) => status_id.oid === appResult.status_id)?.Name;
    res.send({
      fulfillmentText: `Статус заявки: ${statusValue}.\n ${appResult.dataMessage}.`
    });
  } catch (error) {
    console.error("Ошибка при получении данных о заявках из базы данных:", error);
    res.send({ fulfillmentText: "Приношу извинения. Ошибка сервера." });
  }
};

// Экспортируем функцию
export default get_specific_application;